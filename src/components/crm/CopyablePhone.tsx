 ...
 *
 * Shared across every CRM list view (contacts, pipeline, interests,
 * assignments, performance...) so the sale team always has a one-click
 * way to copy a customer's number and paste it into Zalo / dialer.
 *
 * After click:
 ...
 *   - Silently no-ops if clipboard write fails (older browsers / non-
 *     secure context).
 *
 * `compact` renders a smaller version intended to sit as a sub-line
 * under a primary identifier (the customer's name in the list); the
...
export default function CopyablePhone({
  ...
  async function handleCopy(e: React.MouseEvent) {
    // Prevent the click bubbling up to row/card-level handlers (e.g.
    // links opening the contact detail page).
    e.preventDefault();
    e.stopPropagation();
