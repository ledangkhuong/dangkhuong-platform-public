 ...
 * Đăng ký tài khoản + tạo đơn hàng cho landing page /hoclamtoolvideochonguoimoibatdau
 * Body: { full_name, email, phone, password }
 */

const PRODUCT_SLUG = "hoc-lam-tool-video-cho-nguoi-moi-bat-dau";
const PRODUCT_PRICE = 200000; // 200.000đ

function generateOrderCode(prefix: string = "TV", length = 12): string {
...
export async function POST(req: NextRequest) {
  ...
  const rl = await rateLimit(`hoclamtoolvideo-reg:${ip}`, 5, 600);
  if (!rl.allowed) {
  ...
  try {
    ...
    if (emailAlreadyExists) {
      ...
      console.error("[HocLamToolVideo Register] Signup error:", signUpError);
      return NextResponse.json(
    ...
    const productTitle = product?.title || "Học Làm Tool Video Cho Người Mới Bắt Đầu";
    ...
    if (orderError) {
      console.error("[HocLamToolVideo Register] Order error:", orderError);
      return NextResponse.json(
    ...
    // Dedup với Pixel client-side qua eventId (client gửi lên hoặc fallback
    // deterministic theo order.id để retry không nhân đôi event).
    if (order?.id) {
      try {
        const pixelConfig = await getPixelConfigBySlug("hoclamtoolvideo");
        ...
        const capiBase = {
          ...
          userId,
          sourceUrl: "https://dangkhuong.com/hoclamtoolvideochonguoimoibatdau",
        ...
        console.warn("[HocLamToolVideo Register] CAPI error:", capiErr);
      }
    }
    ...
    if (!isExistingUser) {
      try {
        await admin.from("xp_events").insert({
          ...
          action: "register",
          xp_amount: 100,
          meta: { source: "hoclamtoolvideo_landing" },
        });
      } catch { /* Non-critical */ }
    }
    ...
    try {
      ...
      if (!existingSub) {
        await admin.from("subscribers").insert({
          ...
          full_name: full_name?.trim() || "",
          status: "active",
          source: "hoclamtoolvideo_landing",
        });
      }
    } catch { /* Non-critical */ }
    ...
    console.error("[HocLamToolVideo Register Error]", err);
