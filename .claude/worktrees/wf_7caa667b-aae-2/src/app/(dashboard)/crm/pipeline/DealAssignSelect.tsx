...
export default function DealAssignSelect({
  dealId,
  assignedTo,
  salesUsers,
}: DealAssignSelectProps) {
  ...
  async function handleChange(e: React.ChangeEvent<HTMLSelectElement>) {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(`/api/crm/deals/${dealId}/assign`, {
        method: "POST",
  ...
  return (
    <div className="flex flex-col gap-1 w-full">
      <select
        ...
        disabled={submitting}
        className="text-xs rounded-md px-2 py-1 w-full focus:outline-none focus:border-[#D4A843] transition-colors disabled:opacity-60"
        style={{
