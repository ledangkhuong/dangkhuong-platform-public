...
export default function DeletePixelConfigButton({
  configId,
  configName,
}: {
  configId: string;
  ...
  const handleDelete = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/admin/pixel-configs/${configId}`, {
        method: "DELETE",
      });
      if (!res.ok) {
  ...
  return (
    <button
      ...
      disabled={loading}
      title={confirming ? `Bấm lần nữa để xoá "${configName}"` : "Xoá cấu hình"}
